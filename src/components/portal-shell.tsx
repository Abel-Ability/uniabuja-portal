...
export function PortalShell({
  ...
  const nav = [
    { href: "/portal/dashboard", label: "Dashboard", desc: "Overview" },
    ...modules.map((m) => ({ href: m.href, label: m.label, desc: m.description })),
    ...EXTRA_LINKS,
  ];
  ...
  return (
    <div className="min-h-dvh bg-slate/5">
      {/* Desktop sidebar */}
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-72 flex-col bg-brand-strong lg:flex">
        ...
        <div className="border-t border-white/10 p-4 text-[11px] text-white/70">
          <p className="font-semibold text-white/80">{user.fullName}</p>
      ...
      {open ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          <div
            ...
            aria-hidden="true"
          />
          <div className="absolute inset-y-0 left-0 flex w-72 flex-col bg-brand-strong shadow-xl">
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-30 flex items-center justify-between gap-4 border-b border-slate/10 bg-white/95 px-4 py-3 backdrop-blur sm:px-8">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setOpen(true)}
              className="rounded-lg border border-slate/15 p-2 text-slate lg:hidden"
              aria-label="Open menu"
            >
              ☰
            </button>
            <Link href="/" className="text-sm font-medium text-slate/75 hover:text-brand-strong">
              Public site →
            </Link>
          </div>
          <div className="flex items-center gap-3">
